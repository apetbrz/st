...
tsetchar(Rune u, const Glyph *attr, int x, int y)
{
	...
	if (u == ' ' && TSCREEN.buffer[y][x].mode & ATTR_IMAGE &&
	    tgetisclassicplaceholder(&TSCREEN.buffer[y][x])) {
	...
	if (u == IMAGE_PLACEHOLDER_CHAR || u == IMAGE_PLACEHOLDER_CHAR_OLD) {
		TSCREEN.buffer[y][x].u = 0;
		TSCREEN.buffer[y][x].mode |= ATTR_IMAGE;
	}
}
...
void gr_for_each_image_cell(int (*callback)(void *data, Glyph *gp),
	...
	for (int row = 0; row < term.row; ++row) {
		for (int col = 0; col < term.col; ++col) {
			Glyph *gp = &TSCREEN.buffer[row][col];
			if (gp->mode & ATTR_IMAGE) {
				if (callback(data, gp))
					term.dirty[row] = 1;
			}
...
check_control_code:
	...
	if (width == 0) {
		...
		if (term.c.y <= 0 && term.c.x <= 0)
			...
			gp = &TSCREEN.buffer[term.c.y-1][term.col-1];
		else if (term.c.state & CURSOR_WRAPNEXT)
			gp = &TSCREEN.buffer[term.c.y][term.c.x];
		else
			gp = &TSCREEN.buffer[term.c.y][term.c.x-1];
